...
class TestApi:
    ...
    def test_login_jwt(self, test_client):
        """Login for JWT token"""
        ret = test_client.post(AUTH_LOGIN_URL, json={"username": "admin", "password": "secret"})
    ...
    def test_auth_me(self, test_client):
        ...
        headers_jwt = {"Authorization": "Bearer {}".format(TestApi.jwt_token)}
    ...
    def test_authors_put(self, test_client):
        ...
        ret = test_client.put(
            AUTHORS_URL + TestApi.author_1_id, json=TestApi.author_1, headers=headers_jwt
        )
        assert ret.status_code == 200
        ret_val = ret.json
        ret_val.pop("id")
    ...
    def test_authors_put404(self, test_client):
        ...
        ret = test_client.put(AUTHORS_URL + DUMMY_ID, json=TestApi.author_1, headers=headers_jwt)
        assert ret.status_code == 404

    def test_authors_get_filter(self, test_client):
        """GET AUTHORS  WITH FILTERS"""
        ret = test_client.get(
            AUTHORS_URL, query_string={"first_name": TestApi.author_1.get("first_name")}
        )
        assert ret.status_code == 200
        ret_val = ret.json
    ...
    def test_books_post(self, test_client):
        """POST AUTHORS"""
        TestApi.book_1 = {"title": "Ghostbusters", "author_id": TestApi.author_1_id}
        headers_jwt = {
            "Authorization": "Bearer {}".format(TestApi.jwt_token),
        }
    ...
    def test_books_post2(self, test_client):
        """POST BOOKS AGAIN"""
        TestApi.book_2 = {"title": "Ghostbusters2", "author_id": DUMMY_ID}
        headers_jwt = {
            "Authorization": "Bearer {}".format(TestApi.jwt_token),
        }
    ...
    def test_books_put404(self, test_client):
        ...
        # PUT wrong ID -> 404
        ret = test_client.put(BOOKS_URL + DUMMY_ID, json=TestApi.book_1, headers=headers_jwt)
        assert ret.status_code == 404
    ...
    def test_books_delete(self, test_client):
        ...
        # DELETE
        ret = test_client.delete(BOOKS_URL + TestApi.book_1_id, headers=headers_jwt)
    ...
    def test_authors_delete(self, test_client):
        ...
        # DELETE
        ret = test_client.delete(AUTHORS_URL + TestApi.author_1_id, headers=headers_jwt)
        assert ret.status_code == 204
